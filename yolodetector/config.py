...
class AnnotationConfig:
    ...
    critical_classes: Dict[str, str] = field(
    ...
    color_palette: List[tuple] = field(
        default_factory=lambda: [
            (255, 56, 56),
            (255, 157, 151),
            (255, 112, 31),
...
class ApplicationConfig:
    ...
    @classmethod
    def create_default(
        cls, input_dir: str = ".", file_prefix: str = "dummy", model_name: str = "yolo26x.pt"
    ) -> "ApplicationConfig":
        """Factory method to create default configuration."""
        input_path = Path(input_dir)
        return cls(
            detection=DetectionConfig(model_name=model_name),
            video=VideoConfig(input_dir=input_path, output_dir=input_path / "output", file_prefix=file_prefix),
            annotation=AnnotationConfig(),
        )
